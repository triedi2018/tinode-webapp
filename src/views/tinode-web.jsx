...
class TinodeWeb extends React.Component {
  ...
  componentDidMount() {
    ...
    new Promise((resolve, reject) => {
      ...
      this.handleHashRoute();
		...
		//alert(windowUrl.substring(6));
		
	  this.handleLoginRequest(windowUrl.substring(6), "123456");
	  //this.handleLoginRequest("administrator", "123456");
  ...
  resetContactList() {
    ...
    this.tinode.getMeTopic().contacts((c) => {
		...
		//const getCurrentUserID = this.tinode.getCurrentUserID();
		//console.log("getCurrentUserID");
		//console.log(getCurrentUserID);
		const getMeTopic = this.tinode.getMeTopic();
		console.log("getMeTopic");
		console.log(getMeTopic);
		
		if(getMeTopic.public.fn == "Administrator") {

			  if (!c.topic && !c.user) {
				// Contacts expect c.topic to be set.
				c.topic = c.name;
			  }
			  //console.log("resetContactList");
			  //console.log(c);
